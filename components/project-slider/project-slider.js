(() => {
  ...
  function card(project, index, count, variant = 'cover') {
    ...
    return `<article class="project-card project-card--${escape(project.format)}${panel ? ' project-card--panel' : ''}${project.href ? ' project-card--linked' : ''}" ${panel && project.href ? 'data-action-hover' : ''} data-index="${index}" role="listitem" aria-label="${index + 1} из ${count}: ${title}">
      ${panel ? copy : ''}
