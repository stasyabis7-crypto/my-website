...
const DOTS_GAP_REM = 3.125; // на сколько rem точки ниже нижнего края центральной карточки (50px при 390px)
...
// в сторону от центра); extraDropRem — на сколько rem ниже (или, если
// отрицательное, выше) базового расчёта сдвинуть кольцо.
...
export function initProjectSlider(root = document) {
  ...
  // slider.css — вся геометрия веера выводится из --arc-offset). Поворот и
  // сдвиг вниз замораживаются за пределами CLONE_RING_COUNT колец (иначе
  // карточку увело бы за 90° "вверх ногами") — это ВИЗУАЛЬНЫЙ предел дуги
  // (см. clamp в updateStates), а не то, сколько карточек физически лежит
  // в треке. Карточки за этим пределом НЕ прячем ни visibility, ни opacity:
  // они остаются на своей (замороженной) дуге и просто уезжают за границы
  // viewport по горизонтали — сам viewport (overflow:hidden) их естественно
  // обрезает, и никакой отдельной анимации появления/исчезания не нужно —
  // всё, что видно, двигается одним и тем же непрерывным translateX
  // трека. Если карточки прятать (через visibility или даже плавный
  // opacity), то ровно на границе кольца они не подъезжают, а "проступают"
  // отдельным эффектом — это и есть та самая лишняя "анимация появления".
  //
  // Раз карточки больше не прячутся, клонов на каждый край трека должно
  // хватать на ЛЮБУЮ реалистичную ширину экрана (иначе на очень широком
  // мониторе с одной стороны кончатся клоны раньше, чем реальный хвост
  // массива с другой — и дуга перестанет выглядеть симметричной бесконечной
  // лентой). n-1 клонов — это почти вся колода целиком, с запасом
  // покрывает даже сверхширокие мониторы (см. project history/скриншот на
  // 3426px), а видимых по факту (после клэмпа) колец всё равно ровно
  // CLONE_RING_COUNT.
  const CLONE_RING_COUNT = 4;
  const allSlides = [];
  const n = projectCards.length;
  const CLONE_BUFFER = n - 1;
  ...
  function updateTrackClearance() {
    ...
    //
    // Горизонтальную видимость считаем аналитически (позиция кольца в
    // раскладке трека + его же подтяжка), а НЕ через getBoundingClientRect
    // у самой карточки — во время transition (сразу после клика по точке)
    // её геометрия ещё не осела на финальную позицию, и живой замер в этот
    // момент даёт устаревший (обычно заниженный) результат.
    const cardWidth = slides[0].offsetWidth;
    const cardHeight = slides[0].offsetHeight;
    const { step } = measure();
    ...
    for (let ring = 1; ring <= CLONE_RING_COUNT; ring++) {
      const pull = arcPullPercent(ring) / 100; // доля своей ширины, на которую кольцо подтянуто к центру
      const finalCenterX = centerX + ring * step - pull * cardWidth;
      const angleRad = (ring * RING_ROTATE_DEG * Math.PI) / 180;
      const rotatedHalfWidthReach = (cardWidth / 2) * Math.cos(angleRad) + (cardHeight / 2) * Math.sin(angleRad);
      const visibleHorizontally = finalCenterX + rotatedHalfWidthReach > 0 && finalCenterX - rotatedHalfWidthReach < viewportWidth;
  ...
  function updateDotsPosition() {
    // Точки всегда на фиксированном расстоянии под центральной карточкой —
    // не под концом трека, у которого внизу большой (и переменный от
    // RING_OVERRIDES) запас под опущенные боковые кольца. Считаем позицию
    // аналитически (высота трека до карточки + её собственная высота), а
    // не через getBoundingClientRect у самой карточки — та во время
    // transition ещё анимируется, и её геометрия в этот момент не
    // соответствует конечной, "осевшей" позиции.
    const sliderRect = slider.getBoundingClientRect();
    const trackRect = track.getBoundingClientRect(); // translateX на треке не двигает его по Y — верт. позиция стабильна и без transition
    const trackPaddingTop = parseFloat(getComputedStyle(track).paddingTop) || 0;
    const cardHeight = slides[0].offsetHeight;
    const remPx = parseFloat(getComputedStyle(document.documentElement).fontSize);
    const centerBottom = trackRect.top - sliderRect.top + trackPaddingTop + cardHeight;
    dotsWrap.style.top = `${centerBottom + remPx * DOTS_GAP_REM}px`;
  }
