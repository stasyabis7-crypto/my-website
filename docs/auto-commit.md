...
Notes & safety:
- PNG files are converted to lossless WebP and local references updated before staging. Install `cwebp` (`brew install webp` on macOS). Conversion errors prevent the commit; the original stays intact.
